"""Filesystem requests-store backup service and scheduler."""
...
ZIP_EXTENSION = ".zip"
TEMP_FILE_PREFIX = ".requests-backup-"
...
def backup_dir(settings: Settings) -> Path:
    root = Path(settings.requests_root).expanduser()
...
def list_backups(settings: Settings) -> list[Path]:
    ...
    return sorted(
        (
            ...
            if path.is_file() and path.name.startswith(REQUESTS_BACKUP_FILE_PREFIX)
        ),
...
def create_backup(settings: Settings, now: datetime) -> Path | None:
    ...
    if not root.exists():
        return None
    if not root.is_dir():
        raise NotADirectoryError(f"The requests root is not a directory: {root}")

    root = root.resolve()
    directory = backup_dir(settings)
    directory.mkdir(mode=BACKUP_DIR_MODE, parents=True, exist_ok=True)
    ...
    try:
        with zipfile.ZipFile(temp_path, mode="w", compression=zipfile.ZIP_DEFLATED) as archive:
            for current_root, dirnames, filenames in os.walk(root):
                dirnames[:] = sorted(dirname for dirname in dirnames if dirname != directory.name)
                filenames.sort()
                ...
                for filename in filenames:
                    if filename == REQUESTS_HISTORY_FILE_NAME or filename.startswith("."):
                        continue
...
async def run_backup_loop(app: FastAPI) -> None:
    ...
    while True:
        try:
            ...
            logger.exception("Requests auto-backup tick failed")
        first_tick = False
        await asyncio.sleep(seconds_until_poll())


def _build_backup_path(directory: Path, now: datetime) -> Path:
    stem = f"{REQUESTS_BACKUP_FILE_PREFIX}{now.strftime(NOTEBOOK_BACKUP_NAME_FORMAT)}"
...
def _parse_backup_name(name: str) -> tuple[datetime, int] | None:
    if not name.startswith(REQUESTS_BACKUP_FILE_PREFIX) or not name.endswith(ZIP_EXTENSION):
        return None
    suffix_text = name.removeprefix(REQUESTS_BACKUP_FILE_PREFIX).removesuffix(ZIP_EXTENSION)
