...
async def fetch_tree(
    ...
    *,
    ...
    return [
        _map_node(settings, children_by_name[folder])
        for folder in settings.jenkins_root_folders
        if folder in children_by_name
    ]


def derive_status(raw: Mapping[str, Any], settings: Settings) -> JenkinsStatus:
...
def is_stuck(raw: Mapping[str, Any], settings: Settings) -> bool:
