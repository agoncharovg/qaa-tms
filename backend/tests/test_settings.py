    ...
    path.write_text(
        "\n".join(
            [
                ...
                "QAA_GENERATOR_ACTOR=initial-actor",
...
def test_server_settings_are_admin_only(client: TestClient, tmp_path: Path, monkeypatch) -> None:
    ...
    non_admin_put = client.put(
        ...
        json={"qaa_generator_actor": "blocked"},
    )
    ...
    assert admin_get.json()["qaa_generator_service_token_set"] is True
    assert admin_get.json()["qaa_generator_superuser_token_set"] is True
    assert "qaa_generator_service_token" not in admin_get.json()
    assert "qaa_generator_superuser_token" not in admin_get.json()


def test_put_server_settings_masks_tokens_clears_empty_strings_and_refreshes_runtime(
    client: TestClient,
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    body = response.json()
    assert body["qaa_generator_actor"] == "updated-actor"
    assert body["qaa_generator_base_url"] == "http://updated.example/api/v1"
    assert body["qaa_generator_service_token_set"] is False
    ...
    assert read_back.json()["qaa_generator_service_token_set"] is False
